...
part** (`sections/` next to a view, `parts/` next to a flow — where a flow's
`parts/` holds only its steps and `request.md` / `responses.md` /
`open-questions.md` sit next to the index). The other types are single files.
The full contract — index shape, part frontmatter, file names, regeneration
rules — lives in [`split-documents.md`](split-documents.md); read it before
proposing or executing a split, not for everyday classification.
